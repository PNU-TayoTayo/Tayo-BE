...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating lender Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-lender.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating borrower Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-borrower.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/lender/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for lender and borrower"
...
# call the script to create the channel, join the peers of lender and borrower,
...
function createChannel() {
  ...
  scripts/createChannel1.sh sharing $CLI_DELAY $MAX_RETRY $VERBOSE
  scripts/createChannel2.sh car $CLI_DELAY $MAX_RETRY $VERBOSE
}
...
# Sharing 채널
function deployCC1() {
  scripts/deployCC1.sh sharing $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
# Car 채널
function deployCC2() {
  scripts/deployCC2.sh car $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  # stop org3 containers also in addition to lender and borrower, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.lender.example.com docker_peer0.borrower.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/lender/msp organizations/fabric-ca/lender/tls-cert.pem organizations/fabric-ca/lender/ca-cert.pem organizations/fabric-ca/lender/IssuerPublicKey organizations/fabric-ca/lender/IssuerRevocationPublicKey organizations/fabric-ca/lender/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/borrower/msp organizations/fabric-ca/borrower/tls-cert.pem organizations/fabric-ca/borrower/ca-cert.pem organizations/fabric-ca/borrower/IssuerPublicKey organizations/fabric-ca/borrower/IssuerRevocationPublicKey organizations/fabric-ca/borrower/fabric-ca-server.db'
